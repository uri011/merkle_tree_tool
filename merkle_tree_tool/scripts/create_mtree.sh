...
do
    cat ${prefixs_dir}'doc.pre' ${entry} | openssl dgst -sha1 -binary > ${nodes_dir}'node0.'${k}
...
do
    ...
    do
        ...
        then
            cat ${prefixs_dir}'node.pre' ${nodes_dir}'node'${lag_i}.${dbl_j} | openssl dgst -sha1 -binary > ${nodes_dir}'node'${i}.${j}
            echo 'Node data file node'${i}.${j} 'generated - '${nodes_dir}'node'${lag_i}.${dbl_j}
        else
            cat ${prefixs_dir}'node.pre' ${nodes_dir}'node'${lag_i}.${dbl_j} ${nodes_dir}'node'${lag_i}.${dbl_j_plus} | openssl dgst -sha1 -binary > ${nodes_dir}'node'${i}.${j}
