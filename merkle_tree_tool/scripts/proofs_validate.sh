...
cat ${prefixs_dir}'doc.pre' ${docs_dir}${doc} | openssl dgst -sha1 -binary > ./computed_hash_${z}
...
do
    ...
    then
        ...
        then
            cat ${prefixs_dir}'node.pre' ${nodes_dir}'node'${i}.${j} | openssl dgst -sha1 -binary > ./computed_hash_${z_p}
            echo 'Level '${i}' Proof Validation Computed - node'${i}.${j}
        else
            cat ${prefixs_dir}'node.pre' ${nodes_dir}'node'${i}.${j} ./computed_hash_${z} | openssl dgst -sha1 -binary > ./computed_hash_${z_p}
            echo 'Level '${i}' Proof Validation Computed - node'${i}.${j}' + last computed hash'
        fi
    else
        cat ${prefixs_dir}'node.pre' ./computed_hash_${z} ${nodes_dir}'node'${i}.${j} | openssl dgst -sha1 -binary > ./computed_hash_${z_p}
        echo 'Level '${i}' Proof Validation Computed - last computed hash + node'${i}.${j}
    fi
    z=$(($z+1))
done < ${proofs_dir}${filename}

res=$(xxd -p ./computed_hash_${z})

echo -e 'Resulting Computed Hash: '${res}
