...
cat ${prefixs_dir}'doc.pre' ${file_path} | openssl dgst -sha1 -binary > ${nodes_dir}'node0.'${nfiles}
...
do
    ...
    then
        cat ${prefixs_dir}'node.pre' ${nodes_dir}'node'${lag_i}.${dbl_j} ${nodes_dir}'node'${lag_i}.${dbl_j_plus} | openssl dgst -sha1 -binary > ${nodes_dir}'node'${i}.${nnodes}
        echo 'Node data file node'${i}.${nnodes}' modified - '${nodes_dir}'node'${lag_i}.${dbl_j}' + '${nodes_dir}'node'${lag_i}.${dbl_j_plus}
    else
        cat ${prefixs_dir}'node.pre' ${nodes_dir}'node'${lag_i}.${dbl_j} | openssl dgst -sha1 -binary > ${nodes_dir}'node'${i}.${nnodes}
