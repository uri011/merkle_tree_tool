...
do
    graph_docu+='0.'${k}'-'$(cat ${nodes_dir}'node0.'${k} | xxd -p )' '
    txt_docu+='0:'${k}':'$(cat ${nodes_dir}'node0.'${k} | xxd -p )'\n'
    k=$(($k+1))
done
...
do
    ...
    do
        ...
        graph_docu+=${i}'.'${j}'-'$(cat ${nodes_dir}'node'${i}.${j} | xxd -p )' '
        txt_docu+=${i}':'${j}':'$(cat ${nodes_dir}'node'${i}.${j} | xxd -p )'\n'
...
txt_docu='MerkleTree:sha1:'$doc_pre':'$node_pre':'${nfiles}':'$(($levels+1))':'$(cat ${nodes_dir}'node'${levels}.'0' | xxd -p )'\n'$txt_docu
